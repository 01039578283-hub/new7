"""Non-mutating local/public release checks for site7 title changes."""
...
def fetch(url):
    request=Request(url,headers={"User-Agent":"Site7-Title-Release-Check/1.0"})
...
def legacy_checks():
    checks=[]
    for script,arguments in (("audit_site.py",[]),("add_subject_anchor_tocs.py",["--check"])):
        ...
        if script=="audit_site.py" and passed:
            parsed=json.loads(output)
            passed=parsed["html_pages"]==EXPECTED_HTML and not parsed["errors"]
        checks.append(dict(script=script,passed=passed,exitCode=result.returncode,output=output,
